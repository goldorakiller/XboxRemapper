...
int main()
{
...

	{
		...
		for (int i = 0; i < 4; ++i)
		{
			whichKey[i] = ini.getInteger("config", "key_player" + std::to_string(i + 1), 27);		// defaults to escape
			powerOff[i] = ini.getInteger("config", "off_player" + std::to_string(i + 1), 0);			// defaults to not poweroff
		}
	}

	//XINPUT_BATTERY_INFORMATION XBatt;
	...
	for (;;)
	{
		for (DWORD i = 0; i < XUSER_MAX_COUNT; i++)
		{
			...
			if (XInputGetState(i, &state) == ERROR_SUCCESS)
			{
				...
				//	//printf("%d ", (byte)XBatt.BatteryLevel);
